...
/// Minimal pre-v115 shape: a dives table (FK target for the safety tables) and
/// a diver_settings table with at least one column (so the ALTER guard fires).
...
void main() {
  test('v115 creates the safety review tables', () async {
  ...
  test('v115 adds the safety review columns to diver_settings', () async {
    final db = AppDatabase(_dbAt114());
    addTearDown(() => db.close());
  ...
  test('v115 tables accept round-trip rows through the Drift API', () async {
    final db = AppDatabase(_dbAt114());
    addTearDown(() => db.close());
